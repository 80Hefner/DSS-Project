        ...
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
            ...
            switch (value.getEntregaAtual().getOrigem().getClass().getSimpleName()) {
                ...
                case "Localizacao_Transporte":
                    ...
                    stm.executeUpdate(
                            "INSERT INTO robots VALUES ("+value.getId_robot()+", "+value.isDisponivel()+", '"+value.getEntregaAtual().getQr_code()+"', '"+transporteOrigem.getZona().toString()+"',  NULL, NULL, '"+armazenamentoDestino.getZona().toString()+"', "+armazenamentoDestino.getCorredor()+", " +armazenamentoDestino.getSetor()+ ") " +
                                    "ON DUPLICATE KEY UPDATE DISPONIVEL="+value.isDisponivel()+", QR_CODE='"+value.getEntregaAtual().getQr_code()+"', LOCALIZACAO_ORIGEM='"+transporteOrigem.getZona().toString()+"', CORREDOR_ORIGEM=NULL, SETOR_ORIGEM=NULL, LOCALIZACAO_DESTINO='"+armazenamentoDestino.getZona().toString()+"', CORREDOR_DESTINO="+armazenamentoDestino.getCorredor()+", SETOR_DESTINO="+armazenamentoDestino.getSetor() );
                    break;
                ...
                case "Localizacao_Armazenamento":
                    ...
                    stm.executeUpdate(
                            "INSERT INTO robots VALUES ("+value.getId_robot()+", "+value.isDisponivel()+", '"+value.getEntregaAtual().getQr_code()+"', '"+armazenamentoOrigem.getZona().toString()+"', "+armazenamentoOrigem.getCorredor()+", " +armazenamentoOrigem.getSetor()+", '"+transporteDestino.getZona().toString()+"',  NULL, NULL) " +
